...
def propose_structure(brand_key: str, tone: str, item: dict | None = None) -> dict:
...
商品を直接売り込む表現は避け、知識・歴史・選び方の文脈で自然に登場させてください。"""
    ...
    prompt = f"""FIRE KIDS Magazine の「{brand_jp}」ブランド記事（{tone_jp}）を1本企画してください。
{item_block}
SEO 効果が高く、読者が具体的に求めているテーマを選んでください。
  ...
  "keywords": "キーワード1, キーワード2, キーワード3"}}"""
    ...
    if m:
        try:
            ...
            pass
    return {"title": "", "h2s": [], "theme": raw.strip()[:200], "keywords": ""}
...
def generate_article(brand_key: str, tone: str, fk_id: str = "") -> dict:
    ...
    for attempt in range(MAX_REGEN_RETRIES):
        ...
        if attempt < MAX_REGEN_RETRIES - 1:
            structure = revise_structure(brand_key, tone, structure, overlap["flagged"])
    ...
    prompt  = build_article_prompt(brand_key, tone, title, theme, keywords, overlap.get("flagged", []), item=item)
