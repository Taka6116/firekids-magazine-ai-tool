...
# 本番（wsgi が wp_uploader_local.app をパッケージ読み込み）の両方で
# import できるよう scripts/ を sys.path に追加する。
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from wp_common.wp_client import (  # noqa: E402
    WP_HEADERS,
    build_auth,
    fetch_me,
    find_user_id_by_keyword,
    get_category_id_by_name as _wp_get_category_id_by_name,
    get_or_create_tag as _wp_get_or_create_tag,
    upload_media_from_url as _wp_upload_media_from_url,
)
...
# WP_HEADERS は wp_common.wp_client から import（XSERVER WAF 対策の UA 付与）


def get_auth():
    # Application Password のスペース除去込み（wp_common.build_auth）
    return build_auth(WP_USER, WP_APP_PASSWORD)
...
# 以下の WP REST API 処理は wp_common.wp_client に移動（Phase 3）。
# 既存の呼び出しシグネチャを維持するためのラッパー。

def get_or_create_tag(name):
    return _wp_get_or_create_tag(WP_BASE_URL, get_auth(), WP_HEADERS, name)


def get_category_id_by_name(name):
    """カテゴリ名から ID を取得（完全一致）"""
    return _wp_get_category_id_by_name(WP_BASE_URL, get_auth(), WP_HEADERS, name)


def get_writer_user_id():
    """『編集部』を含むWPユーザーのIDを取得（ライター固定）"""
    return find_user_id_by_keyword(WP_BASE_URL, get_auth(), WP_HEADERS, WRITER_NAME_KEYWORD)


def upload_media_from_url(image_url):
    """画像URLからダウンロードしてWPメディアにアップロード、IDを返す"""
    return _wp_upload_media_from_url(WP_BASE_URL, get_auth(), WP_HEADERS, image_url)
...
def health():
    ...
    try:
        r = fetch_me(base, (user, pw), headers=WP_HEADERS, timeout=15)
        if r.ok:
            u = r.json()
            try:
